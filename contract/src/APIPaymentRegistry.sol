 ...
 */
contract APIPaymentRegistry is
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    function registerProvider(
        ...
        require(
            endpointToProvider[endpoint] == address(0),
    ...
    function issueCredential(
        ...
        (bool successProvider, ) = payable(provider).call{value: totalCost}("");
        ...
        if (msg.value > totalCost) {
            ...
            require(successRefund, "Refund excess failed");
        }
     ...
     */
    function verifyAndConsumeCredential(
    ...
    function recordAPICall(
        ...
        if (wasSuccessful) {
            providers[msg.sender].totalRevenue += providers[msg.sender]
                .pricePerRequest;
        }
        ...
        emit APICallRecorded(
            ...
            msg.sender,
            endpoint,
            providers[msg.sender].pricePerRequest
        );
    }
    ...
    function issueRefund(
        ...
        (bool successRefund, ) = payable(caller).call{value: call.amount}("");
        require(successRefund, "Refund transfer failed");

        // Refund excess
        if (msg.value > call.amount) {
            (bool successExcess, ) = payable(msg.sender).call{
                value: msg.value - call.amount
            }("");
            require(successExcess, "Excess refund failed");
        }
    ...
    function getProvider(
        address provider
    ) external view returns (APIProvider memory) {
    ...
    function getCredential(
        bytes32 credentialId
    ) external view returns (PaymentCredential memory) {
    ...
    function getCallHistory(
        address caller
    ) external view returns (APICall[] memory) {
