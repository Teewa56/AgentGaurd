             ...
             <table className="w-full text-left">
               <thead className="bg-secondary/30 text-xs font-bold uppercase text-muted-foreground">
                 <tr>
                   ...
                   <th className="px-6 py-4">Agent Involved</th>
               ...
               <tbody className="divide-y">
                 {[
                   { id: '#10492', agent: 'Marketing Exec v2', type: 'Policy Violation', outcome: '100% Refund', status: 'Resolved' },
                   { id: '#10501', agent: 'Procurement Bot A', type: 'Unauth Amount', outcome: 'Analyzing...', status: 'Arbitrating' },
                   { id: '#10488', agent: 'Inventory Replenisher', type: 'Merchant Dispute', outcome: '0% Refund', status: 'Merchant Protected' },
                   { id: '#10476', agent: 'Customer Support A', type: 'Double Spend', outcome: '100% Refund', status: 'Resolved' },
                 ].map((dispute, i) => (
                   <tr key={i} className="hover:bg-secondary/10 transition-colors">
                     <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-bold text-sm">{dispute.id}</span>
                          <span className="text-[10px] text-muted-foreground uppercase font-medium">Block #1940210</span>
                        </div>
                     </td>
                     <td className="px-6 py-4 text-sm font-medium">{dispute.agent}</td>
                     <td className="px-6 py-4 text-xs font-semibold">
                        <span className="bg-secondary px-2 py-1 rounded-md">{dispute.type}</span>
                     </td>
                     <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                           {dispute.outcome.includes('Refund') ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> : <HelpCircle className="w-3.5 h-3.5 text-amber-500" />}
                           <span className="text-sm font-bold">{dispute.outcome}</span>
                        </div>
                     </td>
                     <td className="px-6 py-4">
           ...
           <div className="flex items-start justify-between gap-8 flex-col lg:flex-row">
              <div className="flex-1 space-y-4">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                       <h3 className="text-lg font-bold italic tracking-tight">Claude 3.5 Sonnet: Case Analysis Insight</h3>
                 ...
                 <div className="bg-secondary/50 rounded-2xl p-6 border-l-4 border-primary">
                    <p className="text-sm font-medium italic leading-relaxed text-foreground/80">
                      "After analyzing metadata for Transaction #10492, I observed that the agent attempted a purchase at 'social-growth.xyz'. The agent's mission narrative specifically limits use to AWS and OpenAI API proxies. This is a clear policy boundary violation. RECOMMENDED: 100% Refund to User Treasury, 10 MNEE Bond Slash for Policy Hard-Failure."
                    </p>
                 </div>
